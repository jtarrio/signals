...
By default, the demodulator sends the demodulated audio to an instance of the [`AudioPlayer`](../src/players/audioplayer.ts) class, which will play this audio on your speakers or headphones. If you want to do something different with the audio, you can provide your own implementation of the [`Player`](../src/demod/player.ts) interface and pass it in the options for the [`Demodulator`](../src/demod/empty-demodulator.ts) constructor.
